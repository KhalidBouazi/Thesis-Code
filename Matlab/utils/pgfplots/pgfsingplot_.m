...
%   ...
function [] = pgfsingplot_(varargin)
...
if ~exist(strcat(folder,filename,fileext),'file')
    ...
    fprintf(fid,'  \\begin{axis}[xlabel=Rang, \r\n');
    fprintf(fid,'               ylabel=Singul\"arwert, \r\n');
    ...
    for i=1:numprintlines
      if set(i) == 1
        fprintf(fid,'    \\addplot[only marks, mark options={solid}, %s, semithick] table{%s};\r\n',cvec{i},strcat(path,linenames{i},'.txt'));
      else
        fprintf(fid,'    \\addplot[only marks, mark options={solid}, %s, semithick] table{%s};\r\n',cvec{i},strcat(path,linenames{i},'-',num2str(1),'.txt'));
        for j=2:set(i)
          fprintf(fid,'    \\addplot[only marks, mark options={solid, fill=white, fill opacity=60}, %s, semithick, forget plot] table{%s};\r\n',cvec{i},strcat(path,linenames{i},'-',num2str(j),'.txt'));
        end
      end
      if numprintlines > 1
