% Usage: pgfloglog(x,y,[name], [x2,y2,[name],...], filename, folder)
%
% Die Funktionsweise von "pgfloglog(.)" ist (fast) die gleiche, wie die 
% von der Matlab-Funktion "loglog(x,y)".
% 
...
% 
% 'x' und 'y' können Vektoren oder Matrizen sein. Die Angabe von 
% 'name' ist optional. 
%
% Die Datei "pgfloglog(.)" kann bedenkenlos mehrmals ausgeführt werden, 
...
%   y2 = 2*x;
%   
%   pgfloglog(x,y1,'myfile.tikz','d:\myfolder');
%   pgfloglog(x,y1,'a',x,y2,'b','myfile.tikz','d:\myfolder');
%   pgfloglog(x,[y1;y2],'a und b','myfile.tikz','d:\myfolder');
%   ...
function [] = pgfloglog(varargin)
...
% x- und y Vektoren und linenames extrahieren
...
if ~exist(strcat(folder,filename,fileext),'file')
    ...
    fprintf(fid,'  \\begin{loglogaxis}[xlabel=$x$, \r\n');
