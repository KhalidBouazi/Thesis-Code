% Usage: pgfsemilogy(x,y,[linename, x2,y2,...], filename, folder)
%
% Die Funktionsweise von "pgfsemilogy(.)" ist (fast) die gleiche, wie die 
% von der Matlab-Funktion "semilogy(x,y)".
% 
...
% optional.
% 
% Die Datei "pgfsemilogy(.)" kann bedenkenlos mehrmals ausgeführt werden, ohne
...
%   y2 = 2*10.^x;
%   
%   pgfsemilogy(x,y1,'myfile.tikz','d:\myfolder');
%   pgfsemilogy(x,y1,'a',x,y2,'b','myfile.tikz','d:\myfolder');
%   pgfsemilogy(x,[y1;y2],'a und b','myfile.tikz','d:\myfolder');
%   ...
function [] = pgfsemilogy(varargin)
...
if ~exist(strcat(folder,filename,fileext),'file')
    ...
    fprintf(fid,'  \\begin{semilogyaxis}[xlabel=$x$, \r\n');
