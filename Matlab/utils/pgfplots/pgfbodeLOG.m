% Usage: pgfbodeLOG(G1,[name],[G2, [name], ...], [w], filename, folder)
% Usage: pgfbodeLOG(mag, phase, freq, [name], filename, folder)
%
% Die Funktionsweise von "pgfbodeLOG(.)" ist die gleiche, wie von der Matlab
% Funktion "bode(G)", mit dem Unterschied, dass die Übertragungsfunktionen
% doppelt-logarithmisch aufgetragen werden. Statt Übertragungsfunktionen G
% können auch direkt Amplitude, Phase und Frequenz übergeben werden.
%
...
%
% Die Datei "pgfbodeLOG(.)" kann bedenkenlos mehrmals ausgeführt werden, 
...
%   G2 = zpk([],[-10],10);
%   
%   pgfbodeLOG(G,'myfile.tikz','d:\myfolder');
%   pgfbodeLOG(G1,'a',G2,'b',{1e0,1e2},'myfile.tikz','d:\myfolder');
%   ...
% 
% ODER
...
% 
%   pgfbodeLOG(mag,phase,freq,'myBode','myfile','d:\myfolder');
% 
function [] = pgfbodeLOG(varargin)
mag = [];
phase = [];
freq = [];
w = [];
...
if ~isobject(arguments{1})
    warning('Wenn der Plot komisch aussieht, ist ggf. "mag" in dB...!');
end
