...
for i = 1:cols(algdata.x0s)
    v0_ = V1(:,1+(algdata.timesteps-1+algdata.delays)*(i-1));
    v0(:,i) = v0_;
    Vi = dmdreconstruct(A,v0_,algdata.timesteps-1+algdata.delays)';
    Vr = [Vr; Vi];
    Vr_ = [Vr_; Vi(algdata.delays+1:end,:)];
    V__ = [V__; V1(:,(algdata.delays+1:rows(Vi))+rows(Vi)*(i-1))'];
...
for i = 1:cols(algdata.xv0s)
    v0_ = Vtest(1+(algdata.horizon+algdata.delays)*(i-1),:)';
    v0v(:,i) = v0_;
    Vi = dmdreconstruct(A,v0_,algdata.horizon+algdata.delays)';
    Vp = [Vp; Vi];
%     Vp_ = [Vp_; Vi(algdata.delays+1:end,:)];
%     Vt = [Vt; Vtest((algdata.delays+1:rows(Vi))+rows(Vi)*(i-1),:)];
